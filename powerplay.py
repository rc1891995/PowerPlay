...
def main():
    ...
    # --- Fetch ---
    fetch_parser = subparsers.add_parser("fetch", help="Fetch or load Powerball draw data")
    fetch_parser.add_argument("--source", choices=["local", "web"], default="local",
                              help="Data source (default: local CSV file)")

    # --- Analyze ---
    analyze_parser = subparsers.add_parser("analyze", help="Analyze draw frequencies")
    analyze_parser.add_argument("--last", type=int, help="Analyze only the last N draws")
    analyze_parser.add_argument("--weekday", help="Filter draws by weekday (e.g. Mon, Wed, Sat)")
    ...
    # --- Recommend ---
    recommend_parser = subparsers.add_parser("recommend", help="Generate Powerball picks")
    recommend_parser.add_argument("--mode", choices=["hot", "cold"], default="hot",
                                  help="Recommendation mode (hot or cold)")
    recommend_parser.add_argument("--count", type=int, default=1,
